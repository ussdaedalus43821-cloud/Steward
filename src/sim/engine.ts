import type { CareerState, GameEvent } from "../types.js";
...
import {
  ...
  isEmergencyManagerTriggered,
  recordYearSummary,
  reputationGainForPromotion,
  reputationLossForEmergencyManager,
} from "./career.js";
...
import {
  applyAnnualPopulationGrowth,
  isEligibleForReclassification,
  resolveReferendum,
  updateVitals,
} from "./growth.js";
import {
  applyMonthlyFiscalFlow,
  closeFiscalYear,
  ensureEnterpriseFunds,
  fundBalanceRatio,
  tickEnterpriseFundsMonthly,
} from "./fiscal.js";
...
import {
  ensureSubJurisdictions,
  simulateSubJurisdictionTick,
  syncParentPopulationFromChildren,
} from "./subJurisdictions.js";

export function tickMonth(career: CareerState, rng: Rng): GameEvent[] {
  career.clockMonth += 1;
  ...
  for (const program of j.econDevPrograms) {
    if (program.active) program.monthsRemaining -= 1;
    if (program.monthsRemaining <= 0) program.active = false;
  }
  ...
  if (career.eventLog.length > 200) career.eventLog.splice(0, career.eventLog.length - 200);

  return events;
}

function runAnnualClose(career: CareerState, rng: Rng): GameEvent[] {
  const j = career.jurisdiction;
  const tierConfig = getTierConfig(j.tier);
  const events: GameEvent[] = [];
  ...
  if (j.pension) annualPensionTick(j.pension, career.economyMultiplier, rng);
  ...
  if (tierConfig.hasSubJurisdictions && tierConfig.subJurisdictionTier) {
    ...
    for (const child of j.subJurisdictions ?? []) simulateSubJurisdictionTick(child, career.economyMultiplier, rng);
  ...
  if (oversightResult.changed) {
    events.push({
      ...
      kind: "oversight",
      severity:
        oversightResult.newLevel === "normal"
          ? "good"
...
function nextTierFor(tier: string): "municipality" | "county" | "state" | "country" {
  if (tier === "municipality") return "county";
  if (tier === "county") return "state";
  return "country";
}

function oversightMessage(level: string): string {
  switch (level) {
    case "warning":
...
export function acceptPromotion(career: CareerState, rng: Rng): void {
  if (!career.pendingPromotionOffer) return;
  const j = career.jurisdiction;
...
export function declinePromotion(career: CareerState): void {
  career.pendingPromotionOffer = undefined;
}

export function startNextJobAfterEmergencyManager(career: CareerState, rng: Rng): void {
...
export function petitionForReclassification(career: CareerState): { ok: boolean; reason?: string } {
  const j = career.jurisdiction;
  if (j.reclassificationPending) return { ok: false, reason: "A referendum is already pending." };
  const next = isEligibleForReclassification(j);
  if (!next) return { ok: false, reason: "Population has not yet reached the next threshold." };
