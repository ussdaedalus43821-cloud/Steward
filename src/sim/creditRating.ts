import type { CreditRating, PlayerJurisdiction } from "../types.js";
import { clamp } from "../utils/format.js";
import { totalDebtOutstanding } from "./bonds.js";

export function computeRevenueGrowthTrend(j: PlayerJurisdiction): number {
...
export interface CreditScoreBreakdown {
  fundBalanceScore: number;
  debtScore: number;
  growthScore: number;
  pensionScore: number | null;
  overall: number;
  rating: CreditRating;
}

export function computeCreditRating(j: PlayerJurisdiction, fundBalanceRatio: number): CreditScoreBreakdown {
  ...
  let pensionScore: number | null = null;
  if (j.pension) {
...
export function ratingForScore(score: number): CreditRating {
