import type { PlayerJurisdiction, TierConfig } from "../types.js";
...
function economicFactor(j: PlayerJurisdiction): number {
  return clamp(j.vitals.economicHealth / 100, 0.35, 1.6);
}

export function ensureEnterpriseFunds(j: PlayerJurisdiction): void {
  if (j.tier !== "municipality") return;
  if (!j.stage || !stageAtLeast(j.stage, "town")) return;
  if (j.enterpriseFunds.some((e) => e.key === "waterSewer")) return;
...
export function tickEnterpriseFundsMonthly(j: PlayerJurisdiction): void {
...
function transferPerCapita(j: PlayerJurisdiction): number {
  ...
  if (j.tier === "county") return 260;
  if (j.tier === "state") return 1800;
  return 0;
}
...
export function computeAnnualRevenue(
...
export function computeAnnualDepartmentExpenditure(j: PlayerJurisdiction): number {
  return j.departments.reduce((sum, d) => sum + d.allocated, 0);
}

export function computeAnnualDebtService(j: PlayerJurisdiction): number {
  return j.bonds.reduce((sum, b) => (b.balance > 0 ? sum + b.annualPayment : sum), 0);
}

export function computeAnnualTransferToCapitalFund(j: PlayerJurisdiction): number {
...
export interface AnnualBudget {
  ...
  departmentExpenditure: number;
  debtService: number;
  transferToCapitalFund: number;
...
export function computeAnnualEconDevCost(j: PlayerJurisdiction): number {
...
export function computeAnnualBudget(
  ...
  const totalExpenditure =
    departmentExpenditure + debtService + transferToCapitalFund + pensionContribution + econDevCost;
  return {
    revenue,
...
export function applyMonthlyFiscalFlow(
  j: PlayerJurisdiction,
  tierConfig: TierConfig,
  economyMultiplier: number
): void {
  ...
  if (Math.abs(delta - expected) > 0.01) {
    throw new Error(
      `Fund balance invariant violated: expected delta ${expected}, got ${delta}`
    );
  }
  ...
  const fullAccrualExpenseMonthly =
    budget.departmentExpenditure / 12 + interestExpenseMonthly + depreciationMonthly + pensionExpenseMonthly;
  j.netPositionTracked += monthlyRevenue - fullAccrualExpenseMonthly;
}

export function closeFiscalYear(j: PlayerJurisdiction, tierConfig: TierConfig): void {
...
export function computeBalanceCheck(j: PlayerJurisdiction): BalanceCheck {
  const cash =
...
export function estimatedAnnualExpenditure(j: PlayerJurisdiction): number {
  return (
    computeAnnualDepartmentExpenditure(j) +
...
export function fundBalanceRatio(j: PlayerJurisdiction): number {
  const denom =
    j.generalFund.lastYearExpenditures || j.generalFund.ytdExpenditures * 12 || estimatedAnnualExpenditure(j);
