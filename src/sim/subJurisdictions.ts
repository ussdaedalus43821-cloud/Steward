import type { CareerTier, CreditRating, PlayerJurisdiction, SimJurisdiction } from "../types.js";
...
const NAME_PARTS_B: Record<CareerTier, string[]> = {
...
function stageForPop(pop: number) {
  for (const stage of GROWTH_STAGE_ORDER) {
    const t = POPULATION_THRESHOLDS[stage];
    if (pop >= t.min && pop < t.max) return stage;
  }
  return "home_rule_city" as const;
}

function generateName(tier: CareerTier, rng: Rng): string {
  const a = rng.pick(NAME_PARTS_A);
  const b = rng.pick(NAME_PARTS_B[tier]);
  return `${a}${b}`;
}

function initialFinanceFor(population: number, tier: CareerTier, rng: Rng) {
  ...
  return {
    ...
    creditRating: ratingForScore(score) as CreditRating,
    debtToRevenue,
  };
}

export function generateSubJurisdictions(childTier: CareerTier, count: number, rng: Rng): SimJurisdiction[] {
  const nodes: SimJurisdiction[] = [];
  for (let i = 0; i < count; i++) {
    const population =
      childTier === "municipality"
...
export function simulateSubJurisdictionTick(node: SimJurisdiction, economyMultiplier: number, rng: Rng): void {
  if (node.subJurisdictions && node.subJurisdictions.length > 0) {
    for (const child of node.subJurisdictions) simulateSubJurisdictionTick(child, economyMultiplier, rng);
  ...
  node.vitals.infrastructureCondition = clamp(
    node.vitals.infrastructureCondition + rng.range(-2.5, 1.8),
    5,
    100
  );
  ...
  node.vitals.economicHealth = clamp(
    node.vitals.economicHealth + (economyMultiplier - 1) * 25 + rng.range(-2, 2),
    10,
    150
  );
  ...
  const score = clamp(
    50 + (node.finance.fundBalanceRatio - 0.16) * 200 - (node.finance.debtToRevenue - 0.4) * 40,
    2,
    99
  );
...
export function ensureSubJurisdictions(j: PlayerJurisdiction, childTier: CareerTier, count: number, rng: Rng): void {
  if (!j.subJurisdictions) {
    j.subJurisdictions = generateSubJurisdictions(childTier, count, rng);
  }
}

export function expandNodeChildren(node: SimJurisdiction, rng: Rng): void {
  if (node.subJurisdictions) return;
  if (node.tier === "country" || node.tier === "state" || node.tier === "county") {
    const childTier: CareerTier = node.tier === "country" ? "state" : node.tier === "state" ? "county" : "municipality";
...
export function findNodeById(nodes: SimJurisdiction[], id: string): SimJurisdiction | null {
...
export function syncParentPopulationFromChildren(j: PlayerJurisdiction): void {
