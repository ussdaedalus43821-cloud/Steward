import type { EconDevProgram, GrowthStage, PlayerJurisdiction } from "../types.js";
...
const BASELINE_EFFECTIVE_RATE: Partial<Record<string, number>> = {
  municipality: 11,
  county: 13,
  state: 6,
  country: 18,
...
export function computeTaxCompetitivenessIndex(j: PlayerJurisdiction): number {
  const baseline = BASELINE_EFFECTIVE_RATE[j.tier] ?? 10;
  const effective =
    j.taxRates.propertyMillRate * 0.6 +
...
export function computeAnnualPopulationGrowthRate(
...
export function updateVitals(j: PlayerJurisdiction, economyMultiplier: number): void {
  const avgQuality = averageServiceQuality(j);
  const v = j.vitals;
  ...
  const target = clamp(
    55 + (avgQuality - 50) * 0.3 + (v.infrastructureCondition - 55) * 0.25 + (economyMultiplier - 1) * 45,
    10,
    130
  );
...
export function applyAnnualPopulationGrowth(
  j: PlayerJurisdiction,
  economyMultiplier: number
): number {
...
export function nextGrowthStage(current: GrowthStage): GrowthStage | null {
  const idx = stageIndex(current);
  if (idx < 0 || idx >= GROWTH_STAGE_ORDER.length - 1) return null;
  return GROWTH_STAGE_ORDER[idx + 1];
}

export function isEligibleForReclassification(j: PlayerJurisdiction): GrowthStage | null {
  if (j.tier !== "municipality" || !j.stage) return null;
...
export function referendumPassProbability(j: PlayerJurisdiction): number {
  const quality = averageServiceQuality(j);
  const fbr = fundBalanceRatio(j);
  return clamp(0.55 + (quality - 50) / 200 + fbr * 0.6, 0.15, 0.95);
}

export function resolveReferendum(j: PlayerJurisdiction, rng: Rng): { passed: boolean; stage?: GrowthStage } {
  if (!j.reclassificationPending) return { passed: false };
...
export function createEconDevProgram(
  label: string,
  annualCost: number,
