import type { CapitalProject, FundingSource } from "../../types.js";
...
const COLUMN_DEFS: { key: CapitalProject["status"]; label: string }[] = [
...
function fundingLabel(source: FundingSource): string {
  switch (source) {
    case "pay_as_you_go":
...
function renderCard(p: CapitalProject): string {
...
export function renderCapitalProjects(container: HTMLElement): void {
  ...
  openModal(
    ...
    (root) => {
      ...
      root.querySelector("#proj-submit")?.addEventListener("click", () => {
        const name = (root.querySelector("#proj-name") as HTMLInputElement).value.trim() || "Unnamed Project";
        const category = (root.querySelector("#proj-category") as HTMLSelectElement).value;
        const cost = Number((root.querySelector("#proj-cost") as HTMLInputElement).value) || 100000;
        const duration = Number((root.querySelector("#proj-duration") as HTMLInputElement).value) || 12;
...
function openFundModal(projectId: string): void {
  const j = store.career.jurisdiction;
  ...
  openModal(
    ...
    (root) => {
      ...
      root.querySelector("#fund-submit")?.addEventListener("click", () => {
        const source = (root.querySelector("#fund-source") as HTMLSelectElement).value as FundingSource;
