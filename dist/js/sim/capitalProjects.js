...
export function proposeProject(j, name, category, estimatedCost, durationMonths, currentMonth) {
    const project = {
        id: nextId("proj"),
        name,
        category,
...
export function fundProject(j, projectId, fundingSource, currentMonth, bondTermYears = 20) {
    const project = j.capitalProjects.find((p) => p.id === projectId);
    if (!project || project.status !== "proposed")
        return { ok: false, reason: "Project not in Proposed status." };
    ...
    if (fundingSource === "go_bond" || fundingSource === "revenue_bond") {
        const bond = issueBond(j, fundingSource === "go_bond" ? "go" : "revenue", project.estimatedCost, bondTermYears, currentMonth, `${fundingSource === "go_bond" ? "GO" : "Revenue"} Bond — ${project.name}`, project.id);
...
export function startProject(j, projectId, currentMonth) {
    const project = j.capitalProjects.find((p) => p.id === projectId);
    if (!project || project.status !== "funded")
        return { ok: false, reason: "Project must be Funded first." };
    project.status = "in_progress";
    project.startMonth = currentMonth;
    return { ok: true };
}
export function cancelProject(j, projectId, currentMonth) {
    const project = j.capitalProjects.find((p) => p.id === projectId);
    if (!project)
...
export function tickProjectsMonthly(j, currentMonth, rng) {
    const completed = [];
    const overrunEvents = [];
...
export function projectsByStatus(j) {
    const groups = {
        proposed: [],
        funded: [],
        in_progress: [],
    ...
    for (const p of j.capitalProjects)
