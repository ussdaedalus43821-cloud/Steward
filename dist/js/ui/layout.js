...
function jobTitle() {
    ...
    if (j.tier === "municipality" && j.stage)
        return STAGE_JOB_TITLE[j.stage];
    if (j.tier === "county")
        return "County Administrator";
    if (j.tier === "state")
        return "State Budget Director";
    return "National Budget Director";
}
function statClass(good, bad) {
    if (bad)
        return "bad";
    if (good)
        return "good";
...
export function renderHeader(container, activeTab, onTabClick) {
    ...
    <div class="topbar">
      ...
      <div>
        ...
        <div class="juris-meta">${jobTitle()} · ${TIER_LABELS[j.tier]}${j.stage ? " · " + STAGE_LABELS[j.stage] : ""}</div>
      </div>
      <div class="spacer"></div>
      <div class="stat"><span class="label">Population</span><span class="value">${formatPopulation(j.population)}</span></div>
      <div class="stat"><span class="label">Fund Balance</span><span class="value ${statClass(false, j.generalFund.fundBalance < 0)}">${formatMoney(j.generalFund.fundBalance)}</span></div>
      ...
      <div class="clock-controls">
        ${SPEEDS.map((s) => `<button data-speed="${s.speed}" class="${career.clockSpeed === s.speed ? "active" : ""}">${s.label}</button>`).join("")}
      </div>
    </div>
  ...
  `;
    container.querySelectorAll("button[data-speed]").forEach((btn) => {
        btn.addEventListener("click", () => {
            store.setClockSpeed(Number(btn.dataset.speed));
        });
    });
...
function oversightBanner(level) {
    if (level === "warning") {
