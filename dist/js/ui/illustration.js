...
function densityFor(input) {
    if (input.stage)
        return DENSITY_BY_STAGE[input.stage];
...
export function renderIllustrationSVG(input) {
    ...
    const parts = [];
    parts.push(`<rect x="0" y="0" width="${width}" height="${height}" fill="${skyOpacity > 0.85 ? "#dceaf7" : "#e4e8ee"}" />`);
    ...
    for (let i = 0; i < buildingCount; i++) {
        ...
        parts.push(`<rect x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" rx="${rx.toFixed(1)}" fill="${fillColor}" stroke="#2c3542" stroke-width="0.6" opacity="0.95" />`);
        ...
        for (let r = 0; r < windowRows; r++) {
            for (let c = 0; c < windowCols; c++) {
                if (rand() > litProbability * 0.8 + 0.15)
                    continue;
                const wx = bx + 2 + c * (bw / windowCols);
                const wy = by + 3 + r * (bh / windowRows);
                const lit = rand() < litProbability;
                parts.push(`<rect x="${wx.toFixed(1)}" y="${wy.toFixed(1)}" width="3" height="4" fill="${lit ? "#ffe38a" : "#3a4250"}" opacity="0.9" />`);
            }
        }
    }
    ...
    for (let i = 0; i < lightCount; i++) {
        ...
        parts.push(`<circle cx="${lx.toFixed(1)}" cy="${ly.toFixed(1)}" r="${(1.4 * glow).toFixed(1)}" fill="#ffd35c" opacity="${clamp(glow, 0.25, 1).toFixed(2)}" />`);
    }
    if (input.activeCapitalProject) {
        const cx = width - 34;
        const cy = groundY - 46;
        parts.push(`<g stroke="#3a3f47" stroke-width="2" fill="none">
    ...
    return `<svg viewBox="0 0 ${width} ${height}" width="100%" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Jurisdiction illustration">${parts.join("")}</svg>`;
}
