export function lineChart(series, width = 560, height = 200) {
    ...
    const x = (i) => padding.left + (i / (n - 1)) * innerW;
    const y = (v) => padding.top + innerH - ((v - minV) / range) * innerH;
    const parts = [];
    parts.push(`<rect x="0" y="0" width="${width}" height="${height}" fill="none" />`);
    const zeroY = y(0);
    parts.push(`<line x1="${padding.left}" y1="${zeroY.toFixed(1)}" x2="${width - padding.right}" y2="${zeroY.toFixed(1)}" stroke="#c8d0dc" stroke-width="1" stroke-dasharray="3,3" />`);
    ...
    parts.push(`<text x="${padding.left - 6}" y="${padding.top + 4}" font-size="9" fill="#8a97aa" text-anchor="end">${formatAxis(maxV)}</text>`);
    parts.push(`<text x="${padding.left - 6}" y="${(padding.top + innerH).toFixed(0)}" font-size="9" fill="#8a97aa" text-anchor="end">${formatAxis(minV)}</text>`);
