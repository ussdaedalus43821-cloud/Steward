...
export function renderBudget(container) {
    ...
    <div class="two-col">
      ...
      <div class="card">
        ...
        <table>
          ...
          <tfoot>
            <tr><td>Net Change</td><td class="num">${formatMoney(budget.netChange)} ${budget.netChange >= 0 ? badge("Surplus", "good") : badge("Deficit", "bad")}</td></tr>
          </tfoot>
        </table>
      </div>
    </div>
  ...
  `;
    container.querySelectorAll("input[data-rate]").forEach((input) => {
...
function renderRateControl(label, key, value, min, max, step, hint) {
