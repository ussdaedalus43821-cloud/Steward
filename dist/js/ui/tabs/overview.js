...
let drillPath = [];
function resolvePathNodes(roots, path) {
    const resolved = [];
    let level = roots;
...
function renderGroupCard(count) {
...
function renderNodeGrid(nodes) {
    ...
    return `<div class="node-grid">${visible.map(renderNodeCard).join("")}${rest > 0 ? renderGroupCard(rest) : ""}</div>`;
}
function renderBreadcrumbs(j, path) {
...
function renderNodeDetail(node) {
    ...
    <div class="card">
      ...
      <div class="subtitle">${node.stage ? STAGE_LABELS[node.stage] : TIER_LABELS[node.tier]} · Population ${formatPopulation(node.population)}</div>
...
export function renderOverview(container) {
  ...
  `;
    container.querySelectorAll(".node-card[data-node-id]").forEach((elm) => {
        elm.addEventListener("click", () => {
            drillPath.push(elm.dataset.nodeId);
            renderOverview(container);
        });
    });
    container.querySelectorAll(".crumb").forEach((elm) => {
        elm.addEventListener("click", () => {
...
function statTileLocal(label, value, sub) {
    return `<div class="card stat-tile"><div class="label">${label}</div><div class="value">${value}</div>${sub ? `<div class="sub">${sub}</div>` : ""}</div>`;
}
function oversightBadge(level) {
    if (level === "normal")
        return badge("Normal", "good");
    if (level === "warning")
        return badge("Warning", "warn");
    if (level === "oversight")
        return badge("State Oversight", "bad");
    return badge("Emergency Mgmt", "bad");
}
function renderSingleJurisdictionOverview(j) {
    const svg = renderIllustrationSVG({
        stage: j.stage,
...
function avgQuality(j) {
    if (j.departments.length === 0)
        return 50;
    return j.departments.reduce((s, d) => s + d.serviceQuality, 0) / j.departments.length;
}
function wireSingleJurisdictionEvents(container) {
    const btn = container.querySelector("#petition-btn");
    if (btn) {
        btn.addEventListener("click", () => {
            const result = store.petitionForReclassification();
            if (!result.ok)
                alert(result.reason ?? "Not eligible yet.");
        });
    }
}
