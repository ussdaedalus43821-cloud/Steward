...
import { computeAnnualBudget, computeAnnualDebtService, computeBalanceCheck, fundBalanceRatio, } from "../../sim/fiscal.js";
...
function ratioRow(label, value, status, note) {
    return `<tr>
    <td>${label}</td>
...
export function renderFinancials(container) {
    ...
    const expenditureGrowth = expHistory.length >= 2 && expHistory[expHistory.length - 2] > 0
    ...
    <div class="card" style="margin-top:16px;">
      ...
      <table>
        ...
        <tbody>
          ${ratioRow("Unrestricted Fund Balance Ratio", formatPercent(fbr * 100), statusFor(fbr >= FUND_BALANCE_BENCHMARK_RATIO * 0.9, fbr >= 0), "GFOA best practice: ≈15–17% (2 months of operating expenditures)")}
          ${ratioRow("Debt Outstanding / Revenue", formatPercent(debtToRevenue * 100), statusFor(debtToRevenue < 0.6, debtToRevenue < 1.0), "Under ~60% is generally comfortable; over 100% draws rating scrutiny")}
          ${ratioRow("Debt Service / Revenue", formatPercent(debtServiceToRevenue * 100), statusFor(debtServiceToRevenue < 0.08, debtServiceToRevenue < 0.15), "Common muni guideline: keep annual debt service under ~10% of revenue")}
          ${ratioRow("Revenue Growth (YoY)", formatPercent(revenueGrowth * 100), statusFor(revenueGrowth >= expenditureGrowth, revenueGrowth >= expenditureGrowth - 0.02), `Expenditures grew ${formatPercent(expenditureGrowth * 100)} over the same period`)}
          ${ratioRow("Revenue per Capita", formatMoneyFull(annualRevenue / Math.max(1, j.population)), "good", `Expenditures per capita: ${formatMoneyFull(budget.totalExpenditure / Math.max(1, j.population))}`)}
          ${j.pension
        ? ratioRow("Pension Funded Ratio", formatPercent(j.pension.fundedRatio * 100), statusFor(j.pension.fundedRatio >= 0.8, j.pension.fundedRatio >= 0.6), "Actuarially sound is generally considered ≥80% funded")
        : ""}
        </tbody>
      </table>
    </div>
