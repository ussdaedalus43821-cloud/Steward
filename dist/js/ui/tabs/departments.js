...
export function renderDepartments(container) {
    ...
    const pensionCard = j.pension
        ...
        <div class="field-row">
          ...
          <div class="field-hint">${j.pension.plannedContribution < j.pension.annualRequiredContribution
            ? "Funding below ARC will grow the unfunded liability over time."
            : "Fully funding the ARC keeps the unfunded liability stable or shrinking."}</div>
        </div>
      </div>`
        : "";
    const enterpriseCards = j.enterpriseFunds
        .map((ef) => `<div class="card">
    ...
    container.querySelectorAll("input[data-dept]").forEach((input) => {
        input.addEventListener("input", () => {
            store.setDepartmentAllocation(input.dataset.dept, Number(input.value));
        });
    });
    container.querySelectorAll("input[data-enterprise]").forEach((input) => {
        input.addEventListener("input", () => {
            store.setEnterpriseFeeRate(input.dataset.enterprise, Number(input.value));
        });
    });
    container.querySelector("#pension-input")?.addEventListener("input", (e) => {
        store.setPensionContribution(Number(e.target.value));
    });
}
